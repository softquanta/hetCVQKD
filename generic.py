...
def composable_key_rate(n_bks, N, n, p, q, r_code, r_m_star, h_m, d_ent, p_ec, e_ent, e_sm, e_hash, e_cor, e_pe, h_k):
    ...
    Delta_AEP = 4 * np.log2(2 ** p + 2) * np.sqrt(np.log2(18 / ((p_ec ** 2) * (e_sm ** 4))))
    r_m = 2 * (h_k - d_ent + r_code * q - p) - h_m
    n_pa = int(2 * p_ec * n_bks * n * p)
...
def code_rate_calculation(k, n_bks, n, b, i, p, q, e):
    """
...
def correlation_estimation(m, x, y, t, h, v, e):
    """
    ...
    :param x: Alice's sacrificed states.
    :param y: Bob's sacrificed states.
    :param t: The estimated channel transmissivity.
    :param h: The setup efficiency.
    :param v: The electronic noise.
    :param e: The estimated excess noise.
    ...
    # Calculate the correlation coefficients from the estimated signal-to-noise ratio and the parameter estimation data
    r = np.sqrt(snr / (1 + snr))
    r_data = np.mean(np.multiply(x, y))
